...
class TestAPIServer:
    ...
    def test_generate_endpoint_success(self, mock_loader, mock_get_runtime, sample_gguf_config):
        ...
        with TestClient(app) as client:
            ...
            # Generate
            response = client.post("/generate", json={"prompt": "Test prompt", "params": None})
    ...
    @patch("slm_packager.api.manager.get_runtime")
    @patch("slm_packager.api.manager.ConfigLoader")
    def test_info_endpoint_with_loaded_model(
        self, mock_loader, mock_get_runtime, sample_gguf_config
    ):
    ...
    def test_generate_endpoint_returns_409_when_manager_busy(self):
        ...
        with patch(
            "slm_packager.api.server.ModelManager.is_loaded",
            ...
            with patch(
                ...
                side_effect=ModelBusyError(
                    "Model is busy loading or unloading. Try again shortly."
                ),
            ):
                with TestClient(app) as client:
                    response = client.post("/generate", json={"prompt": "Hello"})
